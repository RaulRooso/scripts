...
if [ "$current_bg" != "'$target_bg'" ]; then
    gsettings set org.gnome.desktop.background picture-uri-dark "$target_bg" || echo "⚠️ Background file not found, skipping"
...
|| echo "⚠️ Could not set short-cut list, skipping"
...
name 'Ptyxis Terminal' || echo "⚠️ Could not set short-cut name, skipping"
# Command
gsettings set org.gnome.settings-daemon.plugins.media-keys.custom-keybinding:/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/ptyxis/ \
command 'ptyxis' || echo "⚠️ Could not set keybinding, skipping"
# Shortcut
gsettings set org.gnome.settings-daemon.plugins.media-keys.custom-keybinding:/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/ptyxis/ \
binding '<Ctrl><Alt>T' || echo "⚠️ Could not set keyboard short-cut, skipping"
...
current_cursor=$(gsettings get org.gnome.Ptyxis cursor-shape)
    ...
    echo "⚠️ Ptyxis terminal not found, skipping terminal customization"
fi
...
sudo fwupdmgr update -y || echo "⚠️ No firmware updates available or not supported, skipping"
...
if command -v bun >/dev/null 2>&1; then
    ...
    echo "⚠️ Use bun-js.bun if bun is not recognized"
fi
echo '--------------------------------------------------------------------------------'
echo -e "\n⚠️ Setup complete. It's recommended to reboot the system to apply all changes:\n
sudo reboot\n"
#--------------------------------------------------------------------------------
